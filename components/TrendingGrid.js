  ...
  render() {
    if (this.state.orderNowClicked) {
      ...
      if (this.props.customerId) {
        return (
          <Redirect to={"/restaurant/" + this.state.selectedRestaurantLoc + "/" + this.state.selectedRestaurant + "/" + this.state.customerId + "/" + this.state.name} push />
        );
      }
      return (
        <Redirect to={"/restaurant/" + this.state.selectedRestaurantLoc + "/" + this.state.selectedRestaurant} push />
      );
    }
    return (
          ...
          {this.tileData.map(tile => (
            <GridListTile key={tile.img}>
              <div className="imgContainer">
                ...
                <button className="btn" style={{ paddingTop: "0px", paddingBottom: "0px" }}><span className="text-muted" style={{ fontSize: "20px" }}><big><strong>{((tile.ratings) / 20).toFixed(1)}</strong></big></span><small></small></button>
              </div>
