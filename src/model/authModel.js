...
authModel.mUpdateDetails = (user) => {
    ...
    return connection.getAuthentications().then((model) => {
        if (user.address != undefined) {
            return model.updateOne({ email: user.email }, { $set: { address: user.address } }).then((response1) => {
        ...
        if (user.phone != undefined) {
            return model.updateOne({ email: user.email }, { $set: { phone: user.phone } }).then((response1) => {
                console.log("phone")
                if (response1.nModified == 0) return null;
                //calling generateUserId function to generate unique customerid
                // return model.updateOne().then((response2) => {
        ...
        if (user.password != undefined) {
            return model.updateOne({ email: user.email }, { $set: { password: user.password } }).then((response1) => {
